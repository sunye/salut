...
class CacheIT {
    ...
    void testCacheServerSelectionRecord() throws ParseException {
        ...
        when(srv.getServiceInstanceName()).thenReturn(instanceName);
        when(srv.getServerName()).thenReturn(domain);
        when(srv.getTtl()).thenReturn(ttl);
        cache.cache(srv);
    ...
    void testCachePointerRecord() throws ParseException {
        ...
        when(ptr.getServiceInstanceName()).thenReturn(instanceName);
    ...
    void testGetInstancesForService() throws ParseException {
        ...
        cache.cache(ptr);
        assertThat(cache.getInstancesForService(type)).contains(instanceName);
    }
    ...
    void tesGetAddressesForServer() throws ParseException, UnknownHostException {
        ...
        ARecord aRecord = mock(ARecord.class);
        when(aRecord.getServerName()).thenReturn(domain);
        when(aRecord.getAddress()).thenReturn(address);
        when(aRecord.getTtl()).thenReturn(time);

        Inet4Address secondaryAddress = (Inet4Address) InetAddress.getByAddress(new byte[]{127, 0, 0, 1});
        ARecord otherRecord = mock(ARecord.class);
        when(otherRecord.getServerName()).thenReturn(domain);
        when(otherRecord.getAddress()).thenReturn(secondaryAddress);
        when(otherRecord.getTtl()).thenReturn(time);
    ...
    void tesGetAddressesForServerTTLExpire() throws ParseException, UnknownHostException, InterruptedException {
        ...
        ARecord aRecord = mock(ARecord.class);
        when(aRecord.getServerName()).thenReturn(donatelo);
        when(aRecord.getAddress()).thenReturn(address);
        when(aRecord.getTtl()).thenReturn(time);

        ARecord otherRecord = mock(ARecord.class);
        Domain michelangelo = DomainBuilder.parseString("Michelangelo.local");
        when(otherRecord.getServerName()).thenReturn(michelangelo);
        when(otherRecord.getAddress()).thenReturn(address);
        when(otherRecord.getTtl()).thenReturn(TimeToLive.fromSeconds(200));
    ...
    void tesGetAddressesForServerTimeout() throws ParseException, UnknownHostException, InterruptedException {
        ...
        when(aRecord.getServerName()).thenReturn(domain);
        when(aRecord.getAddress()).thenReturn(address);
        when(aRecord.getTtl()).thenReturn(TimeToLive.fromSeconds(1));
        //When:
        cache.cache(aRecord);
